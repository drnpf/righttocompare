...
function LogoButton({ className, onClick }: { className?: string; onClick?: () => void }) {
  return (
    <button onClick={onClick} className={`${className} cursor-pointer hover:opacity-80 transition-opacity`} data-name="Logo Button">
      <div className="absolute aspect-[664/146] left-[18.04%] right-0 top-0" data-name="RightToCompare_name_slogan">
        <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgRightToCompareNameSlogan} />
      </div>
      <div className="absolute aspect-[70/70] left-0 right-[81.96%] top-0" data-name="RightToCompare_icon">
        <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none size-full" src={imgRightToCompareIcon} />
      </div>
    </button>
  );
}
    ...
    <div className="content-stretch flex gap-[20px] items-center relative shrink-0" data-name="Navigation Bar Links">
      ...
      >
        <div className="absolute inset-[4.167%]" data-name="Icon">
          {isDarkMode ? (
            <div className="absolute inset-[-6.818%]" style={{ "--stroke-0": "rgba(255, 255, 255, 1)" } as React.CSSProperties}>
              <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 34 34">
                <path d={svgPaths.p39896bf0} id="Icon" stroke="var(--stroke-0, #FFFFFF)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" />
              </svg>
            </div>
          ) : (
      ...
      <div className="content-start flex flex-wrap gap-[8px] items-center relative shrink-0" data-name="Navigation Bar Pill List">
        ...
        >
          <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[0] not-italic relative shrink-0 text-[#1e1e1e] dark:text-white text-[0px] text-nowrap">
            <p className="font-['Inter:Bold',sans-serif] font-bold leading-none text-[16px] whitespace-pre">Comparison Tool</p>
          </div>
        </button>
        <button 
        ...
        >
          <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[0] not-italic relative shrink-0 text-[#1e1e1e] dark:text-white text-[0px] text-nowrap">
            <p className="font-['Inter:Bold',sans-serif] font-bold leading-none text-[16px] whitespace-pre">Discussions</p>
          </div>
        </button>
        ...
        {isAuthenticated && user ? (
          ...
          >
            <p className="font-['Inter:Bold',sans-serif] font-bold leading-none text-[14px] md:text-[16px] text-white whitespace-nowrap">Sign In</p>
          </button>
        )}
      </div>
    </div>
